# Reconstruction linguistique des chansons

Date de travail : 25 août 2026

## Règles de publication

- La graphie affichée au public et le guide phonétique envoyé à Suno sont deux textes distincts.
- Une suite poétique de mots attestés n'est jamais présentée comme une traduction littérale.
- Les formes du Manuel de l'étudiant 1 et des enseignements attribués à Hélène ou Nicole O'Bomsawin sont prioritaires.
- Une forme provenant de Laurent (1884) ou de Masta (1932) est identifiée comme historique ou consultative.
- Aucun suffixe, pluriel, négatif ou paradigme n'est créé uniquement parce qu'il paraît plausible.
- Un nouvel audio n'est publié qu'après comparaison audible avec le texte chanté.

## 1. Askwa yudali — réintégrée comme archive à valider

Retirée du dictionnaire public le 26 août 2026 en raison de problèmes importants de prononciation dans la maquette IA. Le 27 août, Guillaum a demandé la réintégration du fichier maître comme archive artistique. La page publique rétablit la graphie du dictionnaire dans les paroles, mais identifie clairement les six lignes dont la construction reste à corriger ou à confirmer. La chanson n'est ni une attestation linguistique, ni un modèle officiel de prononciation.

## 2. Skweda — création moderne à valider

Ajoutée le 27 août 2026 à partir du fichier fourni par Guillaum. Les paroles publiques rétablissent la graphie aln8ba du corpus au lieu du guide phonétique Suno. `Kowanodana` est conservé exactement comme le vocable historique non traduit du refrain publié par Henry Lorne Masta en 1932; son emploi dans cette œuvre moderne n'en fait pas un chant historique ou cérémoniel. Les autres lignes sont présentées comme des constructions poétiques à valider.

## 3. Nikw8bi nd'aln8ba8dwa

Nouveau titre : l'ancien `Nikwonbi N'Wonolonwzé` est abandonné. La nouvelle chanson prend pour titre l'exemple pédagogique documenté « Maintenant je parle abénakis ».

### Paroles de la version actuelle

```text
[Intro]
Nikw8bi.
Mina ida!

[Couplet 1]
Nda nd'aln8ba8dwawi.
Nikw8bi nd'aln8ba8dwa.
N'klozi.
N'mikwaldam.

[Refrain]
Aln8ba8dwaw8gan.
Nikw8bi nd'aln8ba8dwa.
Wli agakimzow8gan!
Mina ida!

[Couplet 2]
N'namih8 sips.
N'namito awikhigan.
N'mow8 aples.
N'miji wasawas.

[Couplet 3]
Nd'aliwizi Guillaum.
N'wigi W8linak.
Nd'ai wigw8mek.
N'wal8wzin.

[Finale]
Wôbanaki mikwalda.
N'mikwaldam.
Wli nanawalmezi.
Adio.
```

Traçabilité : les quatre paires du deuxième couplet sont les exemples VTA/VTI du Manuel. `Nda nd'aln8ba8dwawi` et `Nikw8bi nd'aln8ba8dwa` sont conservés ensemble comme paire pédagogique du projet et doivent rester signalés comme formes à confirmer. Le prénom après `Nd'aliwizi` est la substitution explicitement enseignée par le Manuel.

## 4. Wig8damoda

Nouveau titre : l'ancien guide phonétique `Wla Won Gan x Wô-ban Ki-zoss` est abandonné. `Wig8damoda!` est l'impératif collectif documenté « Réjouissons-nous! ».

### État des paroles

Les paroles précédemment associées à ce fichier ont été retirées le 31 août 2026. Elles provenaient des métadonnées du MP3 et ne constituaient pas une transcription vérifiée de ce qui est réellement chanté. L'audio retenu demeure publié, mais aucun texte ne doit lui être associé avant une transcription complète, écoutée et validée ligne par ligne.

## 5. Kikas

Le titre `Kikas` est conservé : il s'agit du mois de mai dans le Manuel. L'ancien refrain incomplet `Ôda n'wanaldam` est entièrement supprimé. Le sens poétique « on n'oublie pas » est remplacé par l'impératif attesté `Mikwalda!` (« Souviens-toi! »).

### Paroles d'apprentissage

```text
[Appel]
Kizos. Aki. Sibo.
Mskikois.
Mikwalda!

[Sigwan]
Sigwan.
Kikas.
Solgonbi.
Mskiko. Wligen.
Mikwalda!

[Niben]
Niben. Kizos. Wloda.
Mskikwimenak.
Mskikois.
Mikwalda!

[Tagw8gw]
Tagw8gw. Kzelomsen.
Skweda. Pekeda.
Spemki. Alakwsak.
Mikwalda!

[Pbon]
Pbon. Mzatanos.
Kakasakw.
Pebon alakws.
Mikwalda!

[Finale]
W8linak. Ndakina.
Wôbanaki mikwalda.
N'mikwaldam.
Wli nanawalmezi.
```

Traçabilité : `Kikas`, `Sigwan`, `Niben`, `Tagw8gw`, `Pbon`, `Mskiko`, `Mskikois`, `Mskikwimenak`, `Kzelomsen`, `Skweda`, `Pekeda`, `Spemki`, `Alakwsak`, `Mzatanos`, `Kakasakw` et `Pebon alakws` figurent dans le corpus du projet avec leur source. `Mikwalda` est l'impératif du Manuel. Aucun négatif n'est reconstruit.

## Contrôle audio obligatoire

Pour chaque nouvelle génération :

1. écouter la piste entière avec le texte source ouvert;
2. refuser toute piste qui ajoute une voyelle après un `W` final;
3. refuser toute piste qui prononce `CH` comme le français ou `J` comme le français;
4. vérifier que les préfixes `N'`, `Nd'`, `K'` et `Kd'` restent attachés au mot;
5. vérifier les fins en `-ak`, `-al`, `-w` et `-t`;
6. ne publier que la piste explicitement choisie après cette écoute.

## Maquettes générées le 25 août 2026

| Chanson | Durée du fichier publié | Génération Suno retenue |
|---|---:|---|
| Nikw8bi nd'aln8ba8dwa | 1:58 | https://suno.com/song/27a8bff1-a867-4c0b-be9c-9d06e3a55e12 |
| Wig8damoda | 3:16 | export original retenu le 2026-08-31 |
| Kikas | 1:19 | https://suno.com/song/86979f10-61bb-459b-a5d2-d28ea9042fe4 |

Les quatre fichiers conservés ont été chargés et démarrés dans Chrome sans erreur. Un repérage automatique de la parole a aussi servi à écarter une variante qui comprimait ou omettait la finale de `Nikw8bi nd'aln8ba8dwa`. Ce repérage vérifie la présence générale des sections; il ne peut pas attester la prononciation aln8ba. Le site qualifie donc explicitement chaque piste d'« audio IA à valider ».

Les anciennes maquettes aln8ba ont été retirées du dépôt public. Une sauvegarde de travail privée est conservée hors du dépôt pour documenter l'amélioration continue; elle ne doit pas être remise en ligne.

### Correction de `Kwaï mziwi`

La première reconstruction de `Wig8damoda` avalait `Kwaï` au début de l'appel et ne laissait entendre que `mziwi`. Elle a été retirée. Dans la génération de remplacement, la graphie publique reste `Kwaï mziwi!`, tandis que le guide vocal privé place `Kwaï!` et `Mziwi!` sur deux attaques distinctes après quatre temps instrumentaux. Deux variantes ont été générées; la première, qui retardait trop longtemps l'entrée vocale, a été rejetée. La seconde conserve une attaque audible avant `mziwi` et devient la maquette publiée. Cette correction technique ne constitue toujours pas une attestation linguistique de la voix Suno.
